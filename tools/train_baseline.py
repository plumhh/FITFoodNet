"""Train DINOv3 linear-probe, full-finetuning, Adapter, and AdaptFormer baselines."""
...
def build_parser(defaults: dict[str, Any] | None = None) -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Train DINOv3 baseline or PEFT comparison model")
    ...
    parser.add_argument(
        ...
        type=str,
        default=defaults.get("baseline_method", defaults.get("method", "linear_probe")),
        choices=["linear_probe", "full_finetune", "adapter", "adaptformer"],
    )
    ...
    parser.add_argument("--grad_clip_norm", type=float, default=defaults.get("grad_clip_norm", 0.5))

    # Outputs.
    parser.add_argument("--output_dir", type=Path, default=defaults.get("output_dir", "outputs/baseline"))
    parser.add_argument("--exp_name", type=str, default=defaults.get("exp_name", ""))
...
def complete_paths(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if not args.exp_name:
        args.exp_name = f"{args.baseline_method}_{args.model_name}"

    output_dir = Path(args.output_dir)
    if args.save_path is None:
        args.save_path = output_dir / f"{args.exp_name}_best.pth"
    if args.class_json is None:
        args.class_json = output_dir / f"{args.exp_name}_class_indices.json"
    if args.metrics_csv is None:
        args.metrics_csv = output_dir / f"{args.exp_name}_metrics.csv"
    if args.summary_json is None:
        args.summary_json = output_dir / f"{args.exp_name}_summary.json"
    return args
    ...
    for epoch in range(args.epochs):
        ...
        if epoch == 0 and torch.cuda.is_available():
            ...
            for i in range(gpu_count):
                ...
                print(f"GPU {i} peak VRAM: {peak_mem_gb:.4f} GB")
                total_peak_memory += peak_mem_gb
            print(f"Total peak VRAM usage: {total_peak_memory:.4f} GB")
